...
def cultural_algorithm_for_k(G: nx.Graph, k: int, pop_size: int = 50, 
                           max_gen: int = 100, mutation_rate: float = 0.1,
    ...
    for generation in range(1, max_gen + 1):
        ...
        # Progress callback
        if progress_callback and (generation % 10 == 0 or conflicts == 0):
...
def find_chromatic_number(G: nx.Graph, pop_size: int = 50, max_gen: int = 100, 
...
def cultural_algorithm(G: nx.Graph, pop_size: int = 50, max_gen: int = 100, 
