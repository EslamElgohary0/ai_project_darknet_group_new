...
class GraphColoringApp:
    ...
    def setup_theme(self):
        ...
        # Configure hover effects for buttons - تغيير لون النص عند Hover
        style.map('TButton',
                 background=[('active', '#3f5e96')],  # لون الخلفية يظل كما هو
                 foreground=[('active', '#ffd700')])  # لون النص يتغير إلى ذهبي
    ...
    def _run_backtracking(self):
        ...
        # إضافة طباعة للمعاملات للتشخيص
        self.results_text.insert(tk.END, f"Running Backtracking with: max_try={max_try}, time_limit={time_limit}s\n")
    ...
    def _display_backtracking_results(self, k, colors, t):
        ...
        if k is not None:
            ...
            self.results_text.insert(tk.END, f"Computation Time: {t:.4f} seconds\n")
    ...
    def _display_cultural_results(self, success, coloring, k, conflicts, total_time):
        ...
        if success:
            ...
            self.results_text.insert(tk.END, f"Chromatic Number: {k}\n")
            self.results_text.insert(tk.END, f"Total Computation Time: {total_time:.4f} seconds\n")
            self.results_text.insert(tk.END, f"\nColoring Assignment:\n")
