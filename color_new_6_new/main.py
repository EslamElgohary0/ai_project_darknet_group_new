...
class GraphColoringApp:
    def __init__(self, root):
        ...
        self.performance_history = []  # لتخزين تاريخ الأداء
    ...
    def run_solver(self):
        ...
        self.performance_history = []  # مسح تاريخ الأداء السابق
    ...
    def _run_backtracking(self):
        ...
        self.last_algorithm_run = {
            ...
            'performance_history': []  # Backtracking لا يحتوي على تاريخ أداء
        }
    ...
    def _run_cultural(self):
        ...
        # إعادة تهيئة تاريخ الأداء
        self.performance_history = []
        
        def progress_callback(gen, conflicts, colors_used):
            # حساب الـ Fitness (سالب عدد النزاعات لأننا نريد تقليل النزاعات)
            fitness = -conflicts
            ...
            self.root.after(0, lambda: self.results_text.insert(
                tk.END, f"Generation {gen}: Conflicts={conflicts}, Colors={colors_used}, Fitness={fitness}\n"
            ))
            self.results_text.see(tk.END)
            
            # طباعة في الـ Terminal
            print(f"Generation {gen}: Conflicts={conflicts}, Colors={colors_used}, Fitness={fitness}")
        ...
        self.last_algorithm_run = {
            ...
            'performance_history': self.performance_history  # إضافة تاريخ الأداء الكامل
        }
    ...
    def _create_report_image(self, file_path):
        ...
        if self.last_algorithm_run['result']['success']:
            # البحث عن بيانات التلوين في أي من المفاتيز
            coloring_data = None
        ...
        # الرسم البياني 3: مخطط للأداء - يختلف حسب نوع الخوارزمية
        ...
        if (self.last_algorithm_run['algorithm'] == 'Cultural Algorithm' and 
            'performance_history' in self.last_algorithm_run and 
            self.last_algorithm_run['performance_history'] and
            len(self.last_algorithm_run['performance_history']) > 1):
            
            # مخطط Fitness Evolution للـ Cultural Algorithm
            history = self.last_algorithm_run['performance_history']
            ...
            # رسم خط الـ Fitness
            ax3.plot(generations, fitness_values, 'o-', color='#14a37f', 
                    linewidth=2, markersize=6)
            
            ax3.set_xlabel('Generation', color='white', fontweight='bold')
            ax3.set_ylabel('Fitness', color='white', fontweight='bold')
            ax3.set_title('Cultural Algorithm Fitness Evolution', 
                         color='white', fontweight='bold', fontsize=12)
            
            # إضافة شبكة
            ax3.grid(True, alpha=0.3, color='gray')
            
            # إضافة أفضل وأخير قيمة
            if len(generations) > 0:
                ax3.scatter(generations[-1], fitness_values[-1], 
                          color='yellow', s=100, zorder=5, label='Final')
                
                best_fitness_idx = fitness_values.index(max(fitness_values))
                ax3.scatter(generations[best_fitness_idx], fitness_values[best_fitness_idx],
                          color='red', s=100, zorder=5, marker='*', label='Best')
                
                ax3.legend(facecolor='#2a3f5f', edgecolor='white', labelcolor='white')
        
        elif self.last_algorithm_run['algorithm'] == 'Backtracking Search':
            # مخطط أعمدة بسيط لـ Backtracking
            time_taken = self.last_algorithm_run['result']['time']
            success = self.last_algorithm_run['result']['success']
            
            metrics = ['Time (s)', 'Success']
            values = [time_taken, 1.0 if success else 0.0]
            
            bars = ax3.bar(metrics, values, 
                          color=['#3f5e96', '#14a37f' if success else '#e74c3c'],
                          alpha=0.7, width=0.6)
            ax3.set_title('Backtracking Search Performance', 
                         color='white', fontweight='bold', fontsize=12)
        ...
        if algorithm_name == 'Cultural Algorithm':
            summary_text += "CULTURAL ALGORITHM\n"
            summary_text += "=" * 20 + "\n\n"
            
            if success:
                summary_text += f"✓ Solution Found\n"
                summary_text += f"Chromatic Number: {self.last_algorithm_run['result']['k']}\n"
                
                if 'performance_history' in self.last_algorithm_run and self.last_algorithm_run['performance_history']:
                    history = self.last_algorithm_run['performance_history']
                    if history:
                        summary_text += f"Generations Run: {len(history)}\n"
                        summary_text += f"Final Fitness: {history[-1]['fitness']}\n"
                        summary_text += f"Final Conflicts: {history[-1]['conflicts']}\n"
            else:
                summary_text += f"✗ No Solution Found\n"
                summary_text += f"Max colors tried: {self.last_algorithm_run['parameters']['max_k']}\n"
        
        elif algorithm_name == 'Backtracking Search':
            summary_text += "BACKTRACKING SEARCH\n"
            summary_text += "=" * 20 + "\n\n"
            ...
            if success:
                ...
                if coloring_data:
                    ...
                    summary_text += f"Colors Used: {unique_colors}\n"
            else:
                summary_text += f"✗ No Solution Found\n"
                summary_text += f"Max colors tried: {self.last_algorithm_run['parameters']['max_colors']}\n"
